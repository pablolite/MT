...
# ROUTER OS 6.x
# CZYSZCZENIE
/ip route rule remove [find comment~"Lan"]
/ip route rule remove [find interface=VLAN202]

# WYJATKI DLA SIEC_IZOLOWANA_WAN
/ip route rule add action=lookup dst-address=($NETWORKvlan201.$MASKvlan24) comment="VLAN202 dostep do LAN" interface=VLAN202 table=main
/ip route rule add action=lookup dst-address=($NETWORKvlan299.$MASKvlan24) comment="VLAN202 dostep do LAN" interface=VLAN202 table=main
/ip route rule add action=lookup dst-address=([/ip address get [find interface=VLAN200] network].$MASKvlan24) comment="VLAN202 dostep do LAN" interface=VLAN202 table=beztunelu
/ip route rule add action=lookup-only-in-table comment="VLAN202 dostep tylko do WAN bez VPN" interface=VLAN202 table=beztunelu
